# CoursebookMD — User Guide

Welcome to **CoursebookMD**, a Markdown-based coursebook viewer and editor. This guide shows you how to use the app and demonstrates the Markdown features it supports.

## How to use this guide

- Use the **chapter sidebar** on the left to jump between chapters
- Click a **group label** (e.g. "Getting Started", "Going Further") in the sidebar to collapse or expand its chapters
- Use the **contents sidebar** on the right to jump within a chapter
- Use the **prev/next** buttons at the bottom of each page
- Press **Present** or `Ctrl+Alt+P` (`⌘+⌃+P` on macOS) to toggle presentation mode for lectures
- Toggle **dark mode** with the switch in the top bar
- **Export HTML** to download a standalone version of the coursebook

## Keyboard shortcuts

| Action                                          | Windows/Linux                 | macOS                         |
| ----------------------------------------------- | ----------------------------- | ----------------------------- |
| Toggle presentation mode                        | `Ctrl+Alt+P`                  | `⌘+⌃+P`                       |
| Toggle editor                                   | `Ctrl+Alt+E`                  | `⌘+⌃+E`                       |
| Toggle dark mode                                | `Ctrl+Alt+I`                  | `⌘+⌃+I`                       |
| Toggle spotlight dimming (in presentation mode) | `S` or `Ctrl+Alt+S`           | `S` or `⌘+⌃+S`                |
| Jump to first section                           | `Home`                        | `Home` or `⌘+↑`               |
| Jump to last section                            | `End`                         | `End` or `⌘+↓`                |
| Exit presentation mode                          | `Esc` or the present shortcut | `Esc` or the present shortcut |

## Chapters

> **Authoring tip:** An H2 or H3 heading immediately before chapter links
> becomes a collapsible group label in the sidebar. The heading below
> (`### Getting Started`) groups the two chapters that follow it.

### Getting Started

- [Getting Started](chapters/01-getting-started.md)
- [Writing Content](chapters/02-writing-content.md)

### Going Further

- [Rich Content](chapters/03-rich-content.md)
- [Present and Export](chapters/04-present-and-export.md)
