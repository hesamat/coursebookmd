...
CoursebookMD fetches previews for external links as soon as a coursebook loads and caches them in memory. Wikipedia links use the Wikipedia REST API; other links use the [Jina AI Reader](https://jina.ai/reader). The popup renders the summary as formatted Markdown, and sign-in or blocked pages are detected and do not produce a popup. Links the preload has not covered yet — added since the last open, missed during a rate-limit cooldown, or in a standalone document — are fetched the first time you hover them.

Links into the same workbook — chapter links rewritten to `#chapter-slug` and raw `#heading` anchors — use no network at all: hovering one shows the target section's title and opening content, read straight from the rendered page. Index locator links work the same way: each one anchors an occurrence of the term, so hovering it previews the subsection the occurrence lives in. The popup's title performs the link's own action instead of opening a new tab — including the index locator's jump-and-flash — and the preview works the same way in the exported HTML viewer. Previews appear after a brief pointer dwell (~0.2 s), so moving the mouse across a link-rich paragraph does not flash popups; keyboard focus still previews immediately.
