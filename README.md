...
- **Live preview on save** — when a coursebook is opened from disk (Chrome/Edge), files edited and saved in an external editor are detected automatically: by default a prompt offers to reload, and with "Auto-reload files changed on disk" enabled in Settings the preview re-renders just the changed section without asking; unsaved in-app edits always win, and structural `coursebook.md` changes reload the coursebook
- **Reload Coursebook** — File → Reload Coursebook re-reads the coursebook from disk without re-opening it: fresh files for everything you have not edited, while unsaved in-app edits are kept (in every mode that can re-read: disk folders, folder fallbacks, and URL-loaded coursebooks). Firefox/Safari cannot see file changes after a folder is opened (the browser only provides selection-time copies), so there the action explains the limitation and a re-open picks up the latest files
