...
Links into the same workbook — chapter links rewritten to `#chapter-slug` and raw `#heading` anchors — use no network at all: hovering one shows the target section's title and opening content, read straight from the rendered page. Index locator links work the same way: each one anchors an occurrence of the term, so hovering it previews the subsection the occurrence lives in. The popup's title follows the link in place instead of opening a new tab, and the preview works the same way in the exported HTML viewer.
